...
- Sustained SDPF metadata: `Voltage_envelope/<scope>/Sustained_SDpf.json`. The same build also writes `Sustained_SDpf_summary.xlsx` with a compact `Ranked cases` sheet (population rank, explicit population, governing fixed LG/LL measurement and phase, applied threshold, normalized excess area, longest continuous duration, sustained `V_T`, event timestamps, qualifying paths, and limit source) and a `Representative selections` sheet for the independently selected highest-`V_T`, cumulative-stress, and longest-duration rows in each population. The ranked sheet includes a small method block with result version, persistence, frequency, and derived cycle coverage; it is an audit/selection workbook, not a plot manifest.
...
- Shared Sustained SDPF fingerprint validation is performed once per scope for multi-voltage heatmap, plot, and report reuse. The project scan and analysis caches write compact JSON; session/autosave data remains readable JSON.
