...
- The `Manual` table uses the shared columns `Apply`, `Case`, `Run`, and `Bus`. New rows are applied by default, editable, and support TSV copy/paste. Run and Bus cells accept comma-, semicolon-, or newline-separated values; normalization expands them into individual exact rules, using the Cartesian product when both cells contain lists.
...
- Project scan and project analysis cache validation uses file size and modification time. The targeted `Case_folder` walk uses `os.scandir`/`DirEntry.stat()` while preserving the existing `.inf`, `Statistic*.out`, `CB_*.out`, and PSCAD-log waveform filters. An external edit that preserves both can remain cached until the relevant stage is forced or its generated cache is removed.

## Current validation state

- Latest recorded full source-level test run: **259 passed**. The test command
...
- Envelope source-inventory benchmark: indexed directory output prefixes reduced the reference project's inventory-plus-manifest setup from about 1.5 + 1.6 seconds to 0.33 + 0.07 seconds, with identical 12,780 source records. The optimized project scan reduced the same reference manifest from about 0.58 seconds to 0.18 seconds with identical file lists.
