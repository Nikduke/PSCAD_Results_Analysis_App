...
| Sustained SDPF | Each raw fixed LG phase and LL pair independently, using complete-cycle peak metrics and a piecewise-linear peak envelope | Qualification is unchanged: a candidate must pass both the physical-duration and consecutive-complete-cycle conditions. Actual SDPF and Safety-margin-only Case/Run/MM populations are selected independently by three optional views: highest voltage sustained for T, normalized full-wave excess area, and longest continuous full-wave duration | Up to three representatives per population (up to six per voltage) for the report/plot, plus all observations in the compact JSON and ranked summary workbook |
...
The project sustained-TOV rule is peak-envelope based. For every complete cycle, the app calculates the maximum absolute sample (positive and negative waveform excursions are treated equally). A cycle qualifies a threshold when that peak is at or above the corresponding plotted peak limit. Consecutive qualifying complete cycles form a candidate run. The timestamps of their cycle peaks define a piecewise-linear peak envelope; threshold crossings are linearly interpolated to produce the event start, end, and duration. A sustained event is registered only when the run contains at least `required_cycles` consecutive complete cycles and the interpolated peak-envelope duration is at least the configured physical duration. A short event crossing a cycle boundary therefore cannot qualify solely because it touches two cycle bins. A sub-threshold envelope interval, missing cycle, or data gap ends the event; a later run after the gap is a separate event. Raw sinusoidal zero crossings do not end an event because the sustained test is not applied to instantaneous signed samples. This deliberately rejects isolated fast-decaying events and leaves them to the existing SFO/AFO analysis. RMS is calculated and retained as a supporting diagnostic, but RMS alone cannot register a sustained TOV.
...
- Sustained SDPF summary rows include all Case/Run/MM observations for audit and manual selection. Only a voltage with at least one qualifying fixed path produces a governing Sustained plot and DOCX Sustained SDPF section; a valid no-candidate cache is not an error.
